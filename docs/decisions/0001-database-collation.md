# 0001 データベースの並びを libc から切り離し、基盤に Debian を使う

## 状態

確定（2026-08-03）

tag に patch を含めるかどうかだけ、2026-08-03 に改めました（`18-bookworm` →
`18.4-bookworm`）。並びを builtin で決めることと、基盤を Debian にすることは変えていません。
...
`postgres:18.4-bookworm` を使います。tag には patch まで書きます。

patch を固定すると、上流の修正が入るたびに手で上げることになります。
それでも固定するのは、同じ tag が別のイメージを指す状態を作らないためです。
`18-bookworm` は 18 系の最新を指し続けるため、いつ引いたかで中身が変わります。
不具合を追うときに、どの版で動かしたのかを後から言えません。
上げるのは、依存の更新と同じ機会にまとめて行います。
