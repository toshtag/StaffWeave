...
形式は CycloneDX JSON です。仕様版は生成ツールが出力する版をそのまま使います。
生成は Syft、検証は cyclonedx-cli で、使う版は `scripts/generate-sbom.sh` と
`scripts/validate-sbom.sh` が digest で固定しています。版をここへ書き写しません。
