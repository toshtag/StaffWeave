...
扱う量は従業員数と運用年数に比例して増えます。結果が正しくても、読む量や往復の
回数が対象と無関係に増える実装は、運用を続けるほど遅くなります。応答の内容は
変わらないため、レビューでもテストでも気付けません。

- 一覧を組み立てるとき、行ごとに問い合わせを足さない。必要な子の行は、対象を
  まとめて 1 回で読み、呼び出し側で振り分ける。
- 読み取りだけの要求で書き込まない。「最後に使った時刻」のように精度が要らない
  記録は、要否を判断してから書く。
- 期間を指定した検索は、期間に比例した行だけを読む。索引の先頭は、問い合わせの
  条件と同じ順に並べる。従業員を指定しない経路のために、日付を 2 番目に置いた
  索引を別に持つ。
- 繰り返しの中で変わらない問い合わせは、繰り返しの外へ出す。

回数は応答からは分かりません。`Queryable` を差し替えて実行された SQL を数える
テストで固定します。入力の大きさで回数が決まる経路は、大きさを変えて比べます。
