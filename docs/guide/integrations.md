...
- HTTP エラー、通信失敗、タイムアウトは送信履歴へ記録し、間を広げながら送り直します。
  はじめは 30 秒、失敗するたびに 3 倍へ広げ、上限は 6 時間です。広げないのは、
  落ちている相手へ数秒ごとに出続けても、相手も自分も消耗するだけだからです。
- 8 回試しても通らなければ諦めます。行は消さずに印を付けます。消すと、届かなかった
  こと自体が分からなくなります。
- 諦めた通知は一覧で読め、直したうえで手で送り直せます
  （`GET /api/webhook-deliveries/abandoned` と、その行の `requeue`）。
- 送り直す意味のある失敗と、送り直しても同じ答えになる失敗は分けています。
  相手が落ちている・混んでいる・時間切れは送り直し、それ以外の 4xx は諦めます。
