...
- 配信は at-least-once です。送信後・完了記録前にワーカーが停止すると同じ通知が再送され得るため、
  受け取り側は `eventId` で重複を取り除いてください。
- HTTP 失敗とタイムアウトの自動再送は、現時点ではありません。結果は送信履歴に残ります。
- 送信先の URL が内部ネットワークを指していないかの検証は、まだ実装していません。
  信頼できる送信先だけを登録してください。
...
マイグレーションを適用するまでワーカーは起動に失敗して再試行を繰り返しますが、
適用後は自動で処理を始めます。
