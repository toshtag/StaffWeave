 ...
 * 一つのワークスペースに 3 つの組織を置く。
 *
 * - 雇用元組織 A: 従業員 A が所属する
 * - 雇用元組織 B: 従業員 B と従業員 C が所属する
 * - 受入組織 H: 誰も雇用しない。従業員 B が配属される
 *
 * 次の 5 者から同じ経路を叩く。
 *
 * - ワークスペース管理者: 全体を見られる
 * - 組織 A の管理者: 雇用元として、組織 A の従業員を見られる
 * - 受入組織 H の管理者: 配属だけを根拠に、従業員 B を見られる
 * - 閲覧範囲を持たない組織管理者: 管理対象を持たない
 * - 一般従業員: 自分だけを見られる
 *
 * 閲覧範囲を持たない組織管理者は 2 通りある。従業員が紐づかない利用者は何も
 * 見られず、従業員が紐づく利用者（従業員 C）は自分の分だけを見られる。
 * 後者は CSV の SQL で本人の条件が効いているかを確かめるためにも使う。
 */
import type {
  AnomalyList,
