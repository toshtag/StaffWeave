 ...
 * 送信ワーカーは別の接続から `claimPending` で取り出す。
 *
 * `claimPending` は Workspace をまたいで走査する。利用者の要求ではなく背景処理であるため、
...
export interface WebhookOutboxRepository {
  ...
  claimPending(input: ClaimPendingInput): Promise<ClaimedWebhookDelivery[]>;
      ...
      const rows = await db.query<ClaimedRow>(
        `WITH claimed AS (
         ...
         SELECT claimed.*, endpoints.url, endpoints.secret_hash, endpoints.active
          ...
          ORDER BY claimed.occurred_at`,
        [input.now, expiresAt, input.limit],
      );
